...
void EntityRenderer::postRender(const Entity& entity, const Vec3& pos, float rot, float a)
{
	...
	if (m_pDispatcher->m_pOptions && m_pDispatcher->m_pOptions->m_fancyGraphics.get() && areShadowsAvailable() && m_shadowRadius > 0.0f && (!entity.isPlayer() || !((const Player&)entity).isLocalPlayer()))
	{
