...
int ButtonTile::getSignal(LevelSource* level, int x, int y, int z, int dir)
{
    ...
    int dir = data & 0x7;
    if (dir == 5 && dir == DIR_YPOS) return true;
    if (dir == 4 && dir == DIR_ZNEG) return true;
    if (dir == 3 && dir == DIR_ZPOS) return true;
    if (dir == 2 && dir == DIR_XNEG) return true;
    if (dir == 1 && dir == DIR_XPOS) return true;
...
void ButtonTile::updateDefaultShape()
{
    ...
    constexpr float f1 = 0.125f;
    constexpr float f2 = 0.125f;
    setShape(0.5f - f, 0.5f - f1, 0.5f - f2, 0.5f + f, 0.5f + f1, 0.5f + f2);
}
